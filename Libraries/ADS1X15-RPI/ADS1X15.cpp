...
bool ADS1X15::begin()     // if ADS recognized -> return 1, else -> 0
{
  _ADS_handler = open(_i2cPath, O_RDWR);
  if (_ADS_handler == -1) 
  {
    std::cerr << "Failed to open I2C device" << std::endl;
    std::cout<< "Are you sure about i2c path: "<< _i2cPath<< std::endl;
    return false;
  }
  if(ioctl(_ADS_handler, I2C_SLAVE, _address) == -1)
  {
...
//$$$ this section changed from orginal source code
bool ADS1X15::isConnected()     // if ADS recognized -> return 1, else -> 0
{
  uint8_t reg = 0x00; // random register
  int i = write(_ADS_handler, &reg, 1);
  return (i == 1);
}
//$$$
...
int16_t ADS1X15::_readADC(uint16_t readmode)
{
  ...
  {
    while ( isBusy() );   //  wait for conversion;.
  }
  else
  {
    //  needed in continuous mode too, otherwise one get old value.
    std::chrono::milliseconds duration(_conversionDelay);
...
{
  uint8_t bufWrite[3] = {reg, (uint8_t)(value >> 8), (uint8_t)(value & 0xFF)};
  int i = write(_ADS_handler, &bufWrite, 3);
  return (i == 3);
}
//$$$
...
{
  uint8_t bufRead[2];
  write(_ADS_handler, &reg, 1);
  int rv = read(_ADS_handler, &bufRead, 2);

  if (rv == 2)
  {
    uint16_t value = bufRead[0] << 8;
    value += bufRead[1];
    return value;
  }
  return 0x0000;
...
//  ADS1013
//$$$ this section changed from orginal source code
ADS1013::ADS1013(uint8_t address, const char* i2c_device_path)
{
  _address = address;
...
//  ADS1014
//
ADS1014::ADS1014(uint8_t address, const char* i2c_device_path)
{
  _address = address;
...
//  ADS1015
//
ADS1015::ADS1015(uint8_t address, const char* i2c_device_path)
{
  _address = address;
...
//  ADS1113
//
ADS1113::ADS1113(uint8_t address, const char* i2c_device_path)
{
  _address = address;
...
//  ADS1114
//
ADS1114::ADS1114(uint8_t address, const char* i2c_device_path)
{
  _address = address;
...
//  ADS1115
//
ADS1115::ADS1115(uint8_t address, const char* i2c_device_path)
{
  _address = address;
